<?php
	...
	*/
	...
	add_filter('gdlr_core_custom_menu_widget_walker', 'kingster_gdlr_core_custom_menu_widget_walker');
	if( !function_exists('kingster_gdlr_core_custom_menu_widget_walker') ){
		function kingster_gdlr_core_custom_menu_widget_walker( $walker ){
			return new kingster_menu_walker();
		}
	}
	...
	// nav menu script
	add_action('init', 'kingster_edit_nav_menu');
	if( !function_exists('kingster_edit_nav_menu') ){
		function kingster_edit_nav_menu(){
	if( class_exists('gdlr_core_edit_nav_menu') ){
		new gdlr_core_edit_nav_menu(array(
			...
			'side-text' => array(
				'title' => esc_html__('Side Text (Custom Menu Widget)', 'kingster'),
				'type' => 'text',
				'depth' => '0'
			),
			'side-text-bg' => array(
				'title' => esc_html__('Side Text Background (Custom Menu Widget)', 'kingster'),
				'type' => 'colorpicker',
				'depth' => '0'
			),
		));
