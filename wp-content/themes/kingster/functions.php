<?php
	...
	*/
	...
	if( is_admin() ){
		define('KINGSTER_FREEMIUS_ITEM_ID', '33779');

		include_once(get_template_directory() . '/admin/freemius/gdl/registration.php');

		// freemius api
		if ( ! function_exists( 'kingster_fs' ) ) {
			// Create a helper function for easy SDK access.
			function kingster_fs() {
				global $kingster_fs;

				if ( ! isset( $kingster_fs ) ) {
					// Include Freemius SDK.
					require_once dirname( __FILE__ ) . '/admin/freemius/start.php';

					$kingster_fs = fs_dynamic_init( array(
						'id'                  => KINGSTER_FREEMIUS_ITEM_ID,
						'slug'                => 'kingster',
						'premium_slug'        => 'kingster',
						'type'                => 'theme',
						'public_key'          => 'pk_e60476ae954c503c111176b129444',
						'is_premium'          => true,
						...
						'menu'                => array(
							'slug'           => 'kingster_freemius',
							'parent'         => array(
								'slug'     	 => 'goodlayers_main_menu',
							),
							'contact'        => false,
							'support'        => false,
							'first-path' 	 => 'admin.php?page=theme_landing',
						),
					) );
				}
	...
	include_once(get_template_directory() . '/learnpress/theme-settings.php');
					...
					new gdlr_core_page_option(array(
						...
						'options' => array(
							'layout' => array(
								...
								'options' => array(
									'custom-header' => array(
										'title' => esc_html__('Select Custom Header', 'kingster'),
										'type' => 'combobox',
										'single' => 'gdlr_core_custom_header_id',
										'options' => array('' => esc_html__('Default', 'kingster')) + gdlr_core_get_post_list('gdlr_core_header')
									),
