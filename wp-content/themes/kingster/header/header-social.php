<?php
	...
	$icon_type = kingster_get_option('general', 'top-bar-social-icon-type', 'font-awesome');
	if( $icon_type == 'font-awesome' ){
		...
		$social_list = array(
			...
			'bluesky' => array('title'=> 'Bluesky', 'icon'=>'fa6b fa6-bluesky'), 
	...
	foreach( $social_list as $social_key => $social_icon ){
		$social_link = kingster_get_option('general', 'top-bar-social-' . $social_key);
		...
		if( !empty($social_link) ){
			echo '<a href="' . esc_attr($social_link) . '" target="_blank" class="kingster-top-bar-social-icon" title="' . esc_attr($social_key) . '" >';
